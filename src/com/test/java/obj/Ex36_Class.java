...
public class Ex36_Class {
	public static void main(String[] args) {
		...
		/*
		 ...
		 5. 인터페이스
		 	- 캡슐화 된 내부와 외부간의 최소한의 연결이 가능 통로 역할
		 	ex) 게임 인터페이스
		 	
		 객체 = 데이터 + 행동 의 집합
		 리모컨 = 데이터(회색, 100g, 8cm x 17cm..) + 헹동(전원 on.off 배터리 넣는다. 뺀다.)
		 
		 홍길동 = 데이터 (나이, 몸무게, 피부색깔, 머리색) + 헹동 (말한다. 먹는다. 잔다. 달린다. 생각한다.)
		...
		// - 배열 단점 > 요소간의 성질을 구분하기 힘들다. > 첨자 사용 > 첨자는 의미를 가지지 못한다. > 몇번째 방에 어떤 데이터를 넣었는지 관리 힘들다. > 배열은 같은 타입의 같은 성질의 데이터 집합
